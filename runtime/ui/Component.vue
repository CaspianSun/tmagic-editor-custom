...
import { handleAnimation, handleRemoveAnimation } from './utils/animation'
...
const executeAnimation = () => {
  ...
  if (el && props.config.animation?.length) {
    el.dataset.animation = props.config.animation
...
onMounted(() => {
  if (props.config.animation?.length) executeAnimation()
})
